...
Coins are managed by `managers/CoinManager.js` and stored in `userProgress.coins`.
...
|-------|-------|
| Daily login | 10 |
| Correct answer | varies (base + speed bonus) |
| Perfect answer | 5 |
...
`CoinManager.initialize()` calls `checkDailyBonus()` on every init. If `userProgress.lastLoginDate` differs from today, 10 coins are awarded and the date is updated.

**New users** have no `lastLoginDate`, so the daily bonus fires on their very first login — giving them 10 coins immediately with a balance of 0 points. This is intentional and acts as an incidental welcome gift. It is not a bug.

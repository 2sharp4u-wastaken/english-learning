...
**Remaining:**
- ⬜ `src/engine/gameManager.ts` (GameManager *data half* — `smartQuestionSelection`/pool scoping/audio limits/save·load·deleteGameState/`recordWordAttempt`/`endGame`/`saveGameScoreToHistory` + session state; course-activity context). Drop all DOM render methods (`loadQuestion`/`nextQuestion`/`setup*Listeners`/toasts/modals). The biggest, highest-risk piece. Plus FU-4.4-nikud cure here (React owns nikud).

...
**Progress (2026-06-09):** spike done → chosen path is **public static deploy (Netlify/Vercel) + PWA layer** (a host gives free HTTPS, auto-solving the mic secure-context blocker; PWA adds install + offline; LAN is the weakest option). **Build-asset prerequisite shipped** (commit `0520a2d`): `publicDir: false` meant `vite build` emitted only the bundled graph, so a built `dist/` (and `vite preview`) 404'd on the 5 legacy `<script>` files + `data/*.json` + `img/` — all referenced by bare/absolute paths outside the module graph. A build-only Vite plugin (`infra1-copy-static-assets`) now copies them into `dist`, making it deployable. **PWA layer also shipped** (hand-rolled — vite-plugin-pwa fights the custom build): `manifest.webmanifest` + `sw.js` (network-first navigation, stale-while-revalidate assets) + icons; confetti self-hosted at `/vendor/` (CDN removed) for true offline; SW registered PROD-only from `src/main.tsx`. Verified in `vite preview` (SW controls, offline reload works, confetti offline); wiring pinned by `src/__tests__/pwa-wiring.test.ts`. **Deploy-readiness of the user DB (2026-06-10):** removed the hard-coded omer/zohar/idan seeding from `bridge/auth.ts` (a public deploy must start with an empty user database; existing devices keep their `users` key) and added a first-run create-profile form to `LoginPage` (`createFirstUser` — the one non-admin-gated creation path; without it an unseeded deploy is a brick, since users are otherwise only creatable behind login). Parent password rotated to an app-only value (the old constant looked personal and ships in the public bundle); per-device parent password = backlog §4 "Tier 2", backend/real secrecy = backlog §6 "Tier 3". **Host hookup DONE (2026-06-10): the app is LIVE at https://lomdim-anglit.netlify.app** (site `lomdim-anglit`, CLI-linked; redeploy = `netlify deploy --prod` — the repo is not dashboard-connected, so a push alone does not deploy). Verified live: app shell + sw.js + data/img/vendor assets all 200, caching headers per `netlify.toml`. **INFRA1 complete**; remaining partner item is a real-device play-test over the live HTTPS URL (mic + PWA install). Status tracked in `backlog.md` §1.

### Slice TIER2: Per-device parent password — ✅ SHIPPED (2026-06-10)

Replaced the hard-coded `ADMIN_PASSWORD` constant in `bridge/auth.ts` (which shipped in the public bundle) with a **per-device parent password created on first protected access**. Why this shape: hooking the wizard into `ParentPasswordModal` itself (mode decided per open by `hasParentPassword()`) means ONE change covers every protected surface — SettingsPage tab gate, settings reset, and all UsersTab gated actions — including **existing devices** (no first-run step, no migration: the wizard simply appears the next time a protected screen is opened).

- **Bridge** (`bridge/auth.ts`): `hasParentPassword()` / `setParentPassword()` / `resetParentPassword()`; `verifyAdminPassword()` now checks the stored hash and returns false when none is set. Stored at the **unprefixed** `parentPassword` key (matches `users`/`currentSession` convention), hashed with the same `btoa(SALT+pw+SALT)` scheme as kid passwords.
- **Modal** (`ParentPasswordModal.tsx`): create mode = enter twice (min 4 chars, must match), stores the hash, then flows into the SAME `onSubmit` so the pending gated action proceeds with no second prompt. Verify mode gains a **"שכחתי סיסמה"** link (inline confirm → wipes only `parentPassword` → flips to create mode). Deliberately resettable without the old password: the gate is against a wandering kid, not an attacker, and a no-reset design risks permanent parent lockout. Client-only ⇒ devtools-bypassable by design; real secrecy = backend (backlog §6 "Tier 3").
- **Edge case closed:** admin-role users auto-unlock the settings tabs (never see the gate), but `AddUserModal` requires typing the parent password inline — with no stored password that was a dead end. The "הוסף משתמש" button now routes through the wizard first (`setup-for-add` pending kind in `UsersTab`).

...
**Remaining (b1 wiring):**
- ⬜ Rewire ~13 bridges' `getMgr`/`getApp`/etc. to import `src/engine/*` instead of `window.*`; boot inversion (React owns startup — instantiate engine, wire `createManagers`/providers, mind the FU-4.1 gating race); then delete `gameLogic.js`/`app.js`/`managers/*` + their `<script>` tags; full suite green; bundle drops below 500 KB. Plus FU-4.4-nikud cure (React owns nikud).

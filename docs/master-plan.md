...
### Follow-up FU-HOME-continue: home "continue" CTA target flips first-login vs refresh — NEXT ITEM (parked 2026-06-05)

**Status: the cosmetic symptom is fixed; the behavioral root cause is parked for the next session.** On the first post-login render of `/#/home` the "continue" hero CTA resolves to the *review/practice* target ("תרגול מילים", 🔁); after a plain refresh (same user/data) it resolves to *Word Journey* ("מסע המילים", 🗺️) — i.e. the suggested next game is non-deterministic across loads.

**Already done (committed):** the layout no longer breaks from this — the practice label was shortened ("תרגול מילים שצריך לחזק" → "תרגול מילים") and the hero band was made robust (`sm:flex-nowrap` + greeting `shrink-0` + action-group `min-w-0`) so a long CTA can never push the greeting onto its own row. See `fix(home): keep the welcome hero on one band…`.

**Root cause to fix:** `getContinueTarget()` (`src/bridge/games.ts:84-111`) returns the practice CTA only when `dueCount > 0` **AND** at least one `REVIEW_ROTATION` game (`['listening','picture-match','true-or-not','reading']`) is unlocked via `progress.gameUnlocks[id]?.unlocked`. That `gameUnlocks` map differs first-login (engine in-memory-seeded defaults → review games look unlocked → practice) vs refresh (persisted/empty → `?.unlocked` falsy → falls through to Word Journey) — the **same** in-memory-vs-persisted gating mismatch as **FU-4.1** above (`project_react_home_gating_persisted`). Compounded by `HomePage.tsx`'s `continueTarget = useMemo(() => getContinueTarget(), [])` computing **once at mount**, so an early read sticks until refresh.

**Fix direction (decide product behavior first):** make `getContinueTarget`'s unlock check use the same stable source the home grid uses (the "absent entry = unlocked" rule, or persisted defaults — same call as FU-4.1's fix (b)), so first-login and refresh agree; optionally recompute `continueTarget` on the `engine-ready` event instead of only at mount. Decide what a returning user with due words *should* see (review vs Word Journey) and pin it with a Playwright case that loads `/#/home` twice (fresh-login vs reload) and asserts the CTA target is identical. Files: `src/bridge/games.ts`, `src/features/home/HomePage.tsx`, engine `gameUnlocks` seeding (`src/engine/appState.ts`). Closely related to FU-4.1 — likely fix both together.

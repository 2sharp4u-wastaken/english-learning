...
export interface PictureMatchLearnFirstProps {
  learnedCount: number
}

export function PictureMatchLearnFirst({ learnedCount }: PictureMatchLearnFirstProps) {
  const navigate = useNavigate()
  const nk = useNikud()
  return (
    <section
      data-testid="picture-match-learn-first"
    ...
    >
      ...
      <p className="text-sm text-[color:var(--slate-300)]">
        {nk('כדי לשחק במשחק התמונות צריך ללמוד לפחות 4 מילים. בוא נמשיך במסע המילים!')}
      </p>
      <button
        type="button"
        onClick={() => navigate('/game/word-journey')}
        data-testid="picture-match-learn-first-cta"

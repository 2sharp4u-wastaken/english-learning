...
import { PictureMatchLearnFirst } from '@/features/games/picture-match/components/PictureMatchLearnFirst'
import {
  abortPronunciationSession,
  beginPronunciationSession,
  finishPronunciationSession,
  ...
  type PronunciationSessionResult,
...
export function PronunciationGamePage() {
  const navigate = useNavigate()
  const { caseMode, showNikud } = useTextPrefs()
  const nk = useNikud()
  const [session, setSession] = useState<PronunciationSessionResult | null>(null)
  ...
  const start = useCallback((opts?: { fresh?: boolean }) => {
    ...
    cancelSpeech()
    releaseRecordingUrl()
    const result = beginPronunciationSession(opts ?? {})
    setSession(result)
  ...
  const current: PronunciationQuestion | null =
    phase === 'finished' || !questions[index] ? null : questions[index]

  const playWord = useCallback(
    (allowOverlap = false) => {
      if (!current) return
      void speakWord(current.word.toLowerCase(), 'pronunciation', { allowOverlap })
    },
    [current],
  )
  ...
  const handleRecordToggle = useCallback(async () => {
    ...
    const outcome = recordPronunciationAttempt(current, result)
    setComparison({
      target: current.word,
    ...
    if (outcome.isCorrect) {
      ...
      ;(async () => {
        try {
          ...
          await speakWord(current.word.toLowerCase(), 'pronunciation')
        } catch {
          /* ignore */
        }
      })()
      ...
      <GameScreenShell header={headerProps} progress={progressProps} footer={footer}>
        {current ? (
          <div key={index} className="flex flex-1 flex-col gap-4">
            <MediaPromptCard
              prompt="הקשב, לחץ על המיקרופון ונסה לבטא את המילה"
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <p
                data-testid="pronunciation-status"
                dir="rtl"
            ...
            {phase === 'answered' && comparison ? (
              ...
              >
                ...
                <div className="flex items-center justify-between text-sm text-[color:var(--slate-300)]">
                  ...
                  <span
                    dir="ltr"
                    data-testid="pronunciation-transcript"
                    className={cn(
                ...
                <div className="flex items-center justify-between text-sm text-[color:var(--slate-300)]">
                  <span>{nk('דיוק')}</span>
                  <span
                    data-testid="pronunciation-accuracy"
                    dir="ltr"

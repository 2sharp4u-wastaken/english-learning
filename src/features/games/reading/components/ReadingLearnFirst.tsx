...
export interface ReadingLearnFirstProps {
  learnedCount: number
}

export function ReadingLearnFirst({ learnedCount }: ReadingLearnFirstProps) {
  const navigate = useNavigate()
  const nk = useNikud()
  return (
    <section
      data-testid="reading-learn-first"
    ...
    >
      ...
      <p className="text-sm text-[color:var(--slate-300)]">
        {nk('כדי לשחק במשחק הקריאה צריך ללמוד לפחות 4 מילים. בוא נמשיך במסע המילים!')}
      </p>
      <button
        type="button"
        onClick={() => navigate('/game/word-journey')}
        data-testid="reading-learn-first-cta"

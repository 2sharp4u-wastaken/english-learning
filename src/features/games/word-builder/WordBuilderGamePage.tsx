...
import { WordBuilderLearnFirst } from './components/WordBuilderLearnFirst'
import {
  abortWordBuilderSession,
  beginWordBuilderSession,
  clearWordBuilderAudioState,
  finishWordBuilderSession,
  loadWordBuilderAudioState,
  recordWordBuilderAnswer,
  saveWordBuilderAudioState,
  type WordBuilderQuestion,
  type WordBuilderSessionResult,
} from '@/bridge/word-builder'
...
export function WordBuilderGamePage() {
  const navigate = useNavigate()
  const { caseMode } = useTextPrefs()
  const [session, setSession] = useState<WordBuilderSessionResult | null>(null)
  ...
  const start = useCallback((opts?: { fresh?: boolean }) => {
    hardResetSpeech()
    cancelSpeech()
    if (opts?.fresh) clearWordBuilderAudioState()
    const result = beginWordBuilderSession(opts ?? {})
    setSession(result)
    if (result.kind === 'ready') {
      ...
      const restored = loadWordBuilderAudioState(result.resumeIndex)
  ...
  const current: WordBuilderQuestion | null =
  ...
  // Auto-play sentence on each new question (voice readiness poll, suppressed on resume).
  useEffect(() => {
    if (!current) return
  ...
  const handleSelect = useCallback(
    (i: number) => {
      ...
      const outcome = recordWordBuilderAnswer(current, chosen)
      setSelectedIndex(i)
      setPhase('answered')
      const fb = getGameFeedback('word-builder', outcome.isCorrect ? 'correct' : 'incorrect')
      setFeedback({
    ...
    [advance, current, phase, shuffled.order],
  )
  ...
  if (session.kind === 'learn-first') {
    return (
      <GameScreenShell header={headerProps}>
        <WordBuilderLearnFirst learnedCount={session.learnedCount} />
      </GameScreenShell>
    )
  }
        ...
        {current ? (
          <div key={index} className="flex flex-1 flex-col gap-4">
            <div className="mx-auto flex max-w-md flex-col items-center gap-3 rounded-3xl border border-white/10 bg-[color:var(--ink-900)]/70 p-5 text-center backdrop-blur">
              <span
                data-testid="word-builder-theme"
                ...
                {current.words.map((w, i) => {
                  if (i === current.blank.position) {
                    ...
                    return (
                      <span
                        key={i}
                        data-testid="word-builder-blank"
                        data-state={

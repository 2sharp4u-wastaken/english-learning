...
export interface WordBuilderLearnFirstProps {
  learnedCount: number
}

export function WordBuilderLearnFirst({ learnedCount }: WordBuilderLearnFirstProps) {
  const navigate = useNavigate()
  return (
    <section
      data-testid="word-builder-learn-first"
    ...
    >
      ...
      <p className="text-sm text-[color:var(--slate-300)]">
        כדי לשחק בבונה משפטים צריך ללמוד לפחות 20 מילים. בוא נמשיך במסע המילים!
      </p>
      <button
        type="button"
        onClick={() => navigate('/game/word-journey')}
        data-testid="word-builder-learn-first-cta"

...
- `parentPassword` — per-device parent (admin) password hash, unprefixed (Tier 2, 2026-06-10). There is NO hard-coded admin password anymore: `verifyAdminPassword()` is false until the `ParentPasswordModal` create wizard runs on first protected access. Any surface that collects the parent password inline (not via the modal) must guard the not-set-up state. Tests seed the hash (`seedParentPassword` in `tests/react-routes.spec.js`). **M12 Slice B + M4 (2026-06-17):** the parent is now a real **admin account** (role `parent`) born in the first-run wizard (`FirstRunWizard`) — its password IS the `parentPassword` (`createParentAccount`). Access = **one-unlock parent mode** (`src/bridge/parentMode.ts`, single elevation source: role OR password+timeout, survives navigation; `useParentPassword` delegates). **Kids see ONLY the unprotected תצוגה settings tab** (parent tabs are hidden, not padlocked, until elevated). The `gameUnlockOverride` setting = the parent "פתיחת כל התכנים" toggle (opens all tiles + expressions read-time). `manager` reserved for M5/M13. Details: `project_m12b_m4_parent_account` memory + `docs/wiring-map.md` "Parent (Admin) Gate".

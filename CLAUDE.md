...
- `parentPassword` — per-device parent (admin) password hash, unprefixed (Tier 2, 2026-06-10). There is NO hard-coded admin password anymore: `verifyAdminPassword()` is false until the `ParentPasswordModal` create wizard runs on first protected access. Any surface that collects the parent password inline (not via the modal) must guard the not-set-up state. Tests seed the hash (`seedParentPassword` in `tests/react-routes.spec.js`). **M12 Slice B + M4 (2026-06-17):** the parent is now a real **admin account** (role `parent`) born in the first-run wizard (`FirstRunWizard`) — its password IS the `parentPassword` (`createParentAccount`). Access = **parent-profile login** (2026-06-17 update): the inline "הגדרות הורה" unlock button + the in-settings `ParentPasswordModal` were removed — a parent reaches protected settings by signing in with their (password-gated) parent profile, which auto-elevates by role. `parentMode.ts`/`useParentPassword` still expose `unlocked` (now role-driven; the password+timeout path is no longer triggered from the Settings UI). **Kids see ONLY the unprotected תצוגה ("המשחק שלי") tab** — and since it's the lone tab, the tab rail isn't rendered for them at all (no single redundant pill, no parent-settings button); just the customization content. The `gameUnlockOverride` setting = the parent "פתיחת כל התכנים" toggle (opens all tiles + expressions read-time). `manager` reserved for M5/M13. Details: `project_m12b_m4_parent_account` memory + `docs/wiring-map.md` "Parent (Admin) Gate".
- `v2_playerPrefs_<userId>` — **per-player** customization ("המשחק שלי": theme/sounds/motion/mascot/avatar), a single JSON blob via `src/bridge/playerPrefs.ts` + `usePlayerPrefs`. **Distinct from the device-global `AppSettings`** (`bridge/settings.ts`) — this is per-profile look-and-feel, applied live (`AppShell` sets `data-theme`/contrast/motion on `#react-root`; `bridge/feedback.ts` gates sounds/confetti; `bigText` = `html.app-big-text`). Designed as one opaque blob to sync unchanged in the future cloud backend. Details: `project_player_customization` memory.
...
- `src/features/games/shared/GameScreenShell.tsx` — hero + header + progress + main + footer slots. Forwards `header.title/icon/subtitle` to `<GameHero>` and the rest of `header` to `<GameHeader>`. **`fitViewport` defaults `true`** (since the 2026-05-27 UX pass): the shell is one screen (`100dvh`), header/title/progress/footer stay pinned, and only `<main>` scrolls if it overflows — so a footer "next" button is never pushed below the fold. New games get this free; pass `fitViewport={false}` only if a game genuinely needs page scroll. **Compact mode (M3, 2026-06-14):** `useCompactViewport()` (`max-height: 600px` → catches small/landscape phones) auto-collapses the chrome — `GameHero` becomes one slim line, paddings tighten, `QuestionProgress` slims — and the shell resets `<main>` scrollTop on `progress.current` change so a prior question's scroll never hides the next. All shared, so every game inherits it; `data-compact` exposes the state for tests. **Landscape two-pane (M3):** pass the prompt/media block as the `prompt` slot (keep the interaction — AnswerGrid etc. — as `children`); in short landscape (`@media (orientation: landscape) and (max-height: 600px)`, `.game-twopane` in globals.css) `<main>` splits prompt | interaction so nothing scrolls. Portrait/no-`prompt` stays single-column. Adopted: Vocabulary/Listening/Picture-Match/True-or-Not (batch 1), Grammar/Fill-Blanks/Articles/Progressive (batch 2), Reading/Sentence-Scramble/ABC/Phonics/Story-Time-quiz (batch 3, 2026-06-16), Word Journey (batch 4 — Discover/ListenMatch/Spell/SayWord stages; Recall is a memory grid, left single-column). **M3 landscape two-pane is now complete across all games.** New "prompt + answers" games should use the `prompt` slot. **Short-landscape chrome hide (2026-06-16b):** in the SAME `(orientation: landscape) and (max-height: 600px)` query, globals.css `display:none`s the decorative chrome — `[data-testid='game-hero']`, `[data-testid='question-progress']`, and WJ's per-stage `.wj-stage-counter` — inside the `[data-compact='true']` shell, so `<main>` (flex-1) fills the short height (the control header stays). One rule, every game; tablet-landscape (height >600) keeps the full chrome. Story Time splits ONLY its quiz phase (read phase = full-width single column). **WJ stages can't use the parent's `prompt` slot (they're self-contained), so they reuse the same `.game-twopane`/`.game-twopane-prompt`/`.game-twopane-interaction` class names on an INNER wrapper — the globals.css selectors aren't scoped to GameScreenShell's `<main>`, so the row-split works on any element with those classes. **Beta round 2 (M3-a/b/c, 2026-06-18):** both `.game-twopane` panes now get `justify-content:center` (interaction was top-aligned — beta #21); `RewardModal` compacts in the same short-landscape query (class hooks `reward-hero`/`reward-stats`/`reward-actions`) + grid-scroll backdrop (M3-a); and in PORTRAIT compact the GameHero+QuestionProgress (wrapped in a `game-chrome` div) auto-hide on scroll-down like an address bar, freeing height (M3-c) — guarded by a 260ms post-toggle mute + an `overflow > chromeHeight+16` engage threshold so it never sticks hidden. See `project_beta_round2_polish`.
